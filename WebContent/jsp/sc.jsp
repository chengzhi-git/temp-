...
<body>
  <div class="panel admin-panel">
    ...
    <div style="text-align:center;" class="panel-head"><strong class="icon-reorder">生产部门人员权限管理</strong> </div>
...
<tr>
<form action="<%=request.getContextPath()%>/pm/bangdingcg1.do" method="post">
...
<td><input type="hidden" name="D2" value="${dList.get(0).DEPTID}"> ${dList.get(0).DEPTNAME}</td>
...
<c:forEach items="${scList}" var="z">
<tr>
<form action="<%=request.getContextPath()%>/pm/scsc.do" method="post">
<td><input type="hidden" name="UID" value="${z.UID}">${z.UID}</td>
<td><input type="hidden" name="DEPTID"value="${z.DEPTID}">${z.DEPTID}</td>
<td><input type="hidden" name="UNAME"value="${z.UNAME}">${z.UNAME}</td>
<td><input type="hidden" name="UAGE"value="${z.UAGE}">${z.UAGE}</td>
