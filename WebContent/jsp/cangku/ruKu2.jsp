...
<html>
  ...
  <body>
	...
	<tr>
		...
		<td>${st.index+1}
		<!-- 入库单ID-->
		<input type="hidden" name="ISID" value="${inStore.ISID}">
		<!-- 隐藏计划ID -->
		<input type="hidden" name="PID" value="${tostorelistTshu.PID}"></td>
		</td>
	
		<!-- 生产计划名称 -->
		<td>${tostorelistTshu.PNAME}
		<!-- 计划单名称不用 
		<input type="hidden" name="PNAME" value="${tostorelistTshu.PNAME}">--></td>
		
		<!-- 商品名称 -->
		<td>${tostorelistTshu.GNAME}
		<!-- 商品名称不用 
		<input type="hidden" name="GNAME" value="${tostorelistTshu.GNAME}">--></td>
		
		<!-- 生产人员 -->
		<td>${tostorelistTshu.UNAME}
		<%-- 生产人员不用
		<input type="hidden" name="UNAME" value="${tostorelistTshu.UNAME}"> --%></td>
		
		<!-- 计划数量 -->
		<td>${tostorelistTshu.PCOUNT}
		<%-- 计划数量不用
		...
		<!-- 选择规格 -->
		<td>
		<input type="hidden" name="ISLGUI" value="${tostorelistTshu.GG}">${tostorelistTshu.GG}</td>
